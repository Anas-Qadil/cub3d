...
/*   Updated: 2022/06/10 16:42:40 by aqadil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (mlx->world->map[var->ipy_add_yo][var->ipx_add_xo] == 1 &&
		mlx->world->map[var->ipy][var->ipx_add_xo] == 0 &&
		mlx->world->map[var->ipy_add_yo][var->ipx] == 0)
			;
	else
	{
		if (check_valid_move(var, mlx->world->map[var->ipy][var->ipx_add_xo]))
			mlx->px += mlx->pdx * 6 * mlx->move_up;
		if (check_valid_move(var, mlx->world->map[var->ipy_add_yo][var->ipx]))
...
{
	if (check_valid_move(var, mlx->world->map[var->ipy][var->ipx_sub_xo]))
		mlx->px -= mlx->pdx * 6 * mlx->move_down;
	if (check_valid_move(var, mlx->world->map[var->ipy_sub_yo][var->ipx]))
