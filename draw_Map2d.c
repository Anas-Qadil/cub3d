...
/*   Updated: 2022/06/11 19:03:54 by aqadil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_map2d_closed_doors(t_map_vars *var, t_data *mlx)
{
	while (var->loopi < mlx->square_size)
	{
		...
		while (var->loopj < mlx->square_size)
		{
			if (mlx->world->map[var->i][var->j] == DOOR_CLOSED
				&& var->loopi % 2 == 0)
...
void	init_vars_map_2d(t_map_vars *var)
{
	var->j = -1;
	var->savei = 0;
	var->savej += var->loopj;
...
void	draw_map_2d(t_data *mlx)
{
	...
	{
		init_vars_map_2d(&var);
		while (++var.j < mlx->map_x)
		{
			var.savei += var.loopi;
			var.loopi = 0;
			if (mlx->world->map[var.i][var.j] == 0
			|| mlx->world->map[var.i][var.j] == NORTH
			|| mlx->world->map[var.i][var.j] == EAST
			|| mlx->world->map[var.i][var.j] == SOUTH
			...
			else if (mlx->world->map[var.i][var.j] == 1
			|| mlx->world->map[var.i][var.j] == DOOR)
